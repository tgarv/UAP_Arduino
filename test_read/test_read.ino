        ...
        if (!authenticated) {
          ...
          if (success)
          {
            ...
            if (currentblock > 3){  // First sector isn't stuff we want to keep for the tag data
              ...
              char temp[17];
              memcpy(temp, data, 16);   // Copy data into temp (and cast to char[])
              Serial.print("Data: "); Serial.println(temp);
              
              int j;
              for(j=0; j<16; j++){  // For each char in temp, add it to the string representation of the whole tag
                char c = temp[j];
                int index = currentblock*16 + j;
                tagString += c;
              }
            }
